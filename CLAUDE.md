...
2. **학습 설명 포함**: 코드 구현 시 사용자의 학습을 위해 구현한 코드에 대한 구체적인 설명을 어렵지 않게 함께 제공한다.
3. **단계별 구현**: 코드 구현 시 과정을 단계별로 나누고 한 단계가 끝나면 사용자에게 다음 단계 진행을 허가받은 후 다음 단계로 넘어간다.
4. 코드래빗의 코드리뷰에 대한 수정 요청 시 해당 문제에 대해 비전공자도 이해할 수 있는 수준으로 문제와 해당 코드를 설명한다. 그리고 수정을 요청했을 때, 무조건 수정하지 말고 수정이 꼭 필요한지, 옳은 리뷰인지 확인하고 수정이 필요하다고 생각되면 한다. 
5. 이슈 & PR 생성 요청 시 .github/ 안의 템플릿에 맞춰 작성한다.
...
기능 브랜치는 `feat/기능명 #이슈번호`, `fix/버그명 #이슈번호` 형태로 분기 후 `dev`에 병합한다.
