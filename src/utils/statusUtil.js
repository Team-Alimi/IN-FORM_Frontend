...
export const getStatus = (apiStatus) => {
  ...
  if (!mapped) return { text: apiStatus ?? "-", color: "text-gray-500 bg-gray-100 border-gray-300" };

  const opt = STATE_OPTIONS.find((o) => o.key === mapped.key);
  if (!opt) return { text: mapped.text, color: "text-gray-500 bg-gray-100 border-gray-300" };
